...
Add `GROQ_API_KEY` to `.env` and run `supabase/studies.sql` once. Reports are generated by sending a `POST` request to `/api/v1/studies?sector=agriculture` — this endpoint is public (called directly by the "Generate Report" button in the UI) and is protected by per-IP rate limiting only, capped at five requests per hour per IP by default (`ANALYSIS_RATE_LIMIT_PER_HOUR`). Previously generated reports remain readable for free via `GET /api/v1/studies`, with no rate limit and no LLM call involved.
