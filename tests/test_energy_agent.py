...
from app.agents import EnergyAgent
...
def test_demo_collection_is_stored(tmp_path: Path) -> None:
    ...
    result = asyncio.run(EnergyAgent(repository).run("demo"))
    rows = repository.list_observations(sector="energy")
